...
class SitUpScreen extends StatefulWidget {
  ...
  SitUpScreen({Key? key, required this.testType}) : super(key: key);

  @override
  _SitUpScreenState createState() => _SitUpScreenState();
}
...
class _SitUpScreenState extends State<SitUpScreen> {
  ...
  //       await DatabaseHelper.instance.getCsvDataGroupedByClass();
  ...
  List<Map<String, dynamic>> _getPointsTable() {
    ...
    if (selectedStudent != null) {
      ...
      currentPointsTable = kData.getPointsTable(
          selectedStudent!.age, selectedStudent!.gender, 1);
  ...
  Widget build(BuildContext context) {
    if (isLoading) {
      return Scaffold(
        appBar: AppBar(title: const Text('Sit Up')),
                      ...
                      child: Container(
                        decoration: BoxDecoration(
                          color: _selectedRegNo == student.regNo
                              ...
                              : student.sitUpReps == -1 ? const Color(0xffF1F1F1) : const Color(0xffC9C9C9),
                                ...
                                children: [
                                  ...
                                  ..._getPointsTable()
                                    ...
                                    final Map<String, dynamic> row =
                                        entry.value;
                                    // final rowColor = getColorForRow(row['reps']);
                                    final rowColor =
  ...
  Color getColorForRow(String range, int reps) {
    ...
    if (isInRange(range, reps)) {
      return const Color(0xffFFD5A1); // Skin color for the current range
    }
    return const Color(0xffF1F1F1); // Default color
  }
        ...
        if (classItem.students[i].regNo == _selectedRegNo) {
          setState(() {
            classItem.students[i] =
                classItem.students[i].copyWith(sitUpReps: reps);
          });
