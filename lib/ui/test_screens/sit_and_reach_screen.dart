...
class SitAndReachScreen extends StatefulWidget {

  final int testType;

  SitAndReachScreen({Key? key, required this.testType}) : super(key: key);

  @override
  _SitAndReachScreenState createState() => _SitAndReachScreenState();
}

class _SitAndReachScreenState extends State<SitAndReachScreen> {
  ...
  List<Map<String, dynamic>> _getPointsTable() {
    ...
    if (selectedStudent != null) {
      ...
      currentPointsTable = kData.getPointsTable(
          selectedStudent!.age, selectedStudent!.gender, 3);
  ...
  Widget build(BuildContext context) {
    if (isLoading) {
      return Scaffold(
        appBar: AppBar(title: const Text('Sit and Reach')),
                          ...
                          children: [
                            Expanded(
                              child: ElevatedButton(
                                ...
                                onPressed: _updateSitAndReachReps,
                                child: const Text(
                                  'Enter',
                                  style: TextStyle(color: Colors.white),
                                ),
  ...
  void _updateSitAndReachReps() async {
        ...
        if (classItem.students[i].regNo == _selectedRegNo) {
          setState(() {
            classItem.students[i] =
                classItem.students[i].copyWith(sitAndReachCm: reps);
          });
